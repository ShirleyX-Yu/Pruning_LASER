...
class GPTJLaser(AbstractLaser):
    ...
    @staticmethod
    def get_edited_model(model, lname, lnum, rate, intervention="rank-reduction", logger=None, in_place=True, use_quality=False, min_diversity=False):
        if in_place:
            model_edit = model
        else:
        ...
        for name, param in model.named_parameters():
            ...
            # for VENDI scores
                ...
                mat_analysis_tensor = deepcopy(param)
                mat_analysis = do_vendi_approx(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1, use_quality=use_quality, min_diversity=min_diversity)
